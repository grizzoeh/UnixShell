...
expand_environ_var(char *arg)
{
	...
	if (strcmp(arg, "$?") ==
	    0) {  //-> Support for pseudo-variables ("magic variables")
	...
	size_t len_env = strlen(env);

	size_t len_arg = strlen(arg);

	if (len_env > len_arg) {
		arg = realloc(arg, (len_env + 1) * sizeof(char));
	}
...
parse_line(char *buf)
{
	...
	if (strlen(right) > 0) {
		r = parse_line(right);  // recursive (support for multiple pipes)
	}
